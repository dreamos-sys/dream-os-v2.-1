/* ============================================
   ...
   Magic Logic Engine
   Version: 2026.1.0 (Revisi Aman)
   ============================================ */

class AIOrchestrator {
  constructor() {
    // Pastikan DREAM global tersedia
    if (typeof DREAM === 'undefined') {
      console.error('❌ [AI] DREAM global object tidak ditemukan! AI dinonaktifkan.');
      return;
    }
    ...
    this.maxHistory = 10;
    // Aman: gunakan optional chaining dan fallback string kosong
  ...
  async queryAI(message) {
    // Jika endpoint kosong, langsung fallback
    if (!this.apiEndpoint || this.apiEndpoint === '/ai') {
      console.warn('⚠️ [AI] No API endpoint, using fallback');
    ...
    try {
      const response = await fetch(this.apiEndpoint, {
        ...
        body: JSON.stringify({
          message,
          context: DREAM.state?.currentModule || 'unknown',
  ...
  async loadSuggestions() {
    ...
    // Gunakan dataset atau event listener lebih aman, 
    // namun untuk statis inline onclick sudah cukup.
    container.innerHTML = suggestions.map(s => 
      `<span class="ai-suggestion" data-quick="${s.replace(/[^\w\s]/gi, '')}">${s}</span>`
    ).join('');

    // Tambahkan event listener dengan delegation
    container.addEventListener('click', (e) => {
      const target = e.target.closest('.ai-suggestion');
      if (target) {
        const text = target.textContent.replace(/[^\w\s]/gi, '').trim();
        this.quickAsk(text);
      }
    });
  }
...
// ============================================
// INITIALIZE AI (dengan pengecekan DREAM)
// ============================================
if (typeof DREAM !== 'undefined') {
  DREAM.ai = new AIOrchestrator();
} else {
  console.error('❌ [AI] DREAM global tidak ada, AI tidak dapat diinisialisasi.');
}
