...
import { 
  ...
  Database, CloudSync, Settings, MonitorPlay, HelpCircle 
} from 'lucide-react';
  ...
  return (
    <div className="w-full min-h-screen bg-[#020617] text-white flex flex-col items-center justify-center p-4 md:p-10 font-sans">
      <header className="mb-10 text-center">
        ...
        <p className="text-[10px] text-white/30 tracking-[0.5em] mt-2 uppercase italic">Sovereign Edition • Bismillah</p>
      </header>
      ...
      <div className={`grid gap-4 md:gap-6 w-full max-w-7xl ${isTV ? 'grid-cols-6' : 'grid-cols-3'}`}>
        {modules.map((m, i) => (
          <motion.button
            key={i}
            whileHover={{ scale: 1.1, backgroundColor: 'rgba(255, 215, 0, 0.1)', zIndex: 10 }}
            whileTap={{ scale: 0.9 }}
            className="aspect-square flex flex-col items-center justify-center bg-white/5 rounded-[20px] md:rounded-[35px] border border-white/10 shadow-2xl group transition-all duration-300 outline-none focus:ring-2 focus:ring-[#FFD700]"
          >
